...
- Une page `index.html` possède un `<div id="root"> </div>`
  - le `div` est vide
- le script de démarrage se nomme `index.js`
...
Dans l'objet ci-bas, l'url de l'image est séparé en 4 parties: base URL, `imageId`, `imageSize`, et l'extention de fichier.
...
```js
const artistes = personnalités.filter((p) => p.profession === 'artiste');
```
...
Dans la phase commit, on compare par position. Il faut modifier tous les `<li>`, et ensuite ajouter un élément à la fin. C'est pas optimal, et tous les éléments sont regénérés dans le DOM.
...
Dans la phase commit, on compare par clé. Il suffit d'ajouter l'élément identifié par la clé `{9}`, toutes les autres clés demeurent identiques.
...
- `key` n'est PAS une prop, mais une caractéristique utilisée par React. Si la composante
  a aussi besoin de cette info, il faut alors créer une prop avec un autre nom.
...
Avec le `StrictMode`, quand on est en mode développement, chaque composant est rendu 2 fois. Ça rend (parfois) les effets de bord plus visibles.
...
Il est parfois nécessaire d'utiliser des effets de bord. On peut alors utiliser les gestionnaires d'événements (event handlers), tel que `onClick` ou `onChange`.

Aussi, il est possible de référer une fonction dans `useEffect`, qui exécute alors immédiatement après le rendu. (Similaire à un gestionnaire d'événement, **après** le rendu.)
