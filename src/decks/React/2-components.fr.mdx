...
```js index.js active
import('./styles.css'); // syntaxe Vite.

//TODO: afficher l'heure avec ToLocaleTimeString()
...
// the "commit" phase will determine the minimal DOM update, by comparing the
// current and previous renders. See the inspect tools in browser
...
Aussi, il est possible de référer une fonction dans `useEffect`, qui exécute alors immédiatement après le rendu. (C'est comme un gestionnaire d'événement. après le render.)
...
```js RangeeTemoignage.js active
export default function RangeeTemoignage({ temoignages }) {
  temoignages.push({
    id: 'create',
...
```js App.js hidden
import { useState, useEffect } from 'react';
import RangeeTemoignage from './RangeeTemoignage.js';

let temoignageInitiaux = [
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      <RangeeTemoignages témoignages={temoignages} />
    </div>
  );
}
