...
- Expérience à batir un service Backend (ASP.NET Web API? Azure Functions? Python Flask? Laravel? Express? Autre?)
...
- Il faut donc utiliser une _boîte à outil_ (Vite, Astro) ou un _Framework_ (Next, Gatsby, Remix, TanStack Start)
- Un _MetaFramework_ (Epic Stack, T3, Refine, Redwood, Meteor, Ionic, Wasp)

---

## Historique de React

- Créé en 2011 par Jordan Walke chez Faceboook-Meta
- Utilisé en 2012 pour réécrire Instagram.
- 2013: React est lancé, devient Open Source
- 2015: React Native permet de développer des apps mobiles
- 2017: React 16 (Fiber rendering)
- 2020: React 17 (Hooks)
- 2022: React 18 (permet le “concurrent mode”)
...
- **SPA** Compatible avec le Single Page Application
...
- **Certains composants ne sont pas compatibles aux versions antérieures ou postérieures** Il faut alors se mettre à jour
- **Il faut se faire des opinions** Il faut choisir un bibliothèque de routage, une autre de chargement, une autre pour le caching, une autre pour les styles... C'est fatiguant!
...
- Voici les extensions utiles:
  ...
  - **Prettier** (esbenp.prettier-vscode): formatte le code web (css, js, html)
    {/* prettier-ignore */}
  {/*
  - **ES7+ React/Redux/React-Native snippets** (dsznajder.es7-react-js-snippets): insère des snippets de code
    - Snippets utiles: - **imr, imrs, imrse:** import React, \{useState, useEffect\} - **rfc** Créer composant react - **uef, usf:** useEffect, useState
      {/* prettier-ignore */}
...
- Utilisez `cmd-,` (commande-virgule) pour aller en mode 'settings'
  ...
  - **Editor:** Linked Editing: true (permet de renommer paires de balises)
  - **Editor:** Tab Size: 2 ou 4
  - **Editor:** Format On Save: true
  - **Prettier:** Jsx Single Quote: true
  - **Prettier:** Single Quote: true
  - **Emmet:** Include Language: `"javascript": "javascriptreact"` et `"typescript": "typescriptreact"`
...
- **public**: contient les fichiers qui sont publiés sur le serveur web, sans être compilés par Vite. (png, jpg, pdf, etc)
...
- C'est aussi adopté par d'autres générateurs:
  ...
  - **Svelte** (Productif)
  - **Solid** (Composable)
- JSX n'est pas intégré au standard JavaScript (Un jour?)

---

## JSX dans les coulisses (1/3)

Voici comment un programmeur génère du JSX:

<Sandpack s="70">

```jsx
const alarmTime = '10h40';

const element = <h3 title='alarme'>L'alarme sonne à {alarmTime}</h3>;
...
const alarmTime = '10h41';

const element = createElement(
  'h3',
  { title: 'alarme' },
  "L'alarme sonne à l'heure suivante : ",
  alarmTime
);
...
Note: le transpilateur utilise `_jsx` plutôt que `createElement`, depuis Reaxt 17.

---

## JSX dans les coulisses (3/3)
...
On peut alors naviguer vers http://localhost:4173 (<kbd>o</kbd> et <kbd>enter</kbd> démarre le navigateur)

**Note:**

- pas de **Hot Module Reloading** (HMR)
- Si on change du code, il faut faire un nouveau **build** et ensuite **recharger** la page
