...
Un _Effect_ permet de générer des effets de bord causé par le rendu, plutôt qu'un événement particulier.

Ces _Effects_ roulent suite à la phase "commit", suite à l'affichage des modifications du DOM.

**Note:** On utilisera _Effect_ pour parler de la fonctionalité offerte par `useEffect`.

---

## Pas besoin d'Effects

**On n'a pas toujours besoin d'un _Effect_ ** C'est utile pour se synchroniser avec un système externe. Mais c'est trop si c'est pour modifier le state en fonction d'un autre state.
...
Imaginons le composant `<VideoPlayer>`, et la prop `isPlaying`, pour démarrer ou arrêter la vidéo.
...
La balise `<video>` n'a pas de prop `isPlaying`. Par contre, on a les méthodes `play()` et `pause()`.

Il faut donc synchroniser `isPlaying` avec les appels `play()` et `pause()`

Il faut un `ref` au communiquer avec `<video>` dans le DOM
