...
`Connecting...` apparaît deux fois car, en mode _dev_ avec `<StrictMode>`, le rendu initial roule deux fois, **incluant** les _Effects_.

---

## Explications

Le composant `ChatRoom` est une application ayant plusieurs écrans. L'utilisateur démarre avec la page `ChatRoom`. Au montage (mount), le composant appelle `connection.connect()`. Par la suite,l'utilisateur navigue vers la page `paramètre`. Le composant `ChatRoom` démonte (unmount). Enfin, l'utilisateur retourne à `ChatRoom`, montant à nouveau le composant. Ceci créé une nouvelle connexion, mais la premièere connexion n'a jamais été détruite. À mesure qu'on navigue, on accumule les connexions.

Ces bogues sont subtils et difficiles à trouver. Mais en mode <StrictMode>, c'est plus facile à trouver, car on monte le composant deux fois successivement.

Afficher `"✅ Connecting..."` deux fois de suite permet de saisir qu'on a oublié de fermer la connexion au démontage.

Il faut donc créer une fonction de cleanup (nettoyage) dans l'effet.
...
## React appelle la fonction _cleanup_ juste avant que le prochain Effect n'exécute, et une dernière fois avant que le composant soit démonté (au moment où l'on retire celui-ci.)
...
React runs this Effect, which connects to the `'general'` chat room.à
