...
- Un effet permet de rouler du code après le rendu
...
- **Code de rendu** (rendering code) au top niveau du composant.

  - On prend l'état et le state, on les transforme, et on retourne du JSX.
  - Le code doit demeurer pur.
  - Ce code "calcule" le résultat, sans effet de bord, comment une formule mathématique

- **Gestionnaires d'événements** (Event handler) Ce sont des fonctions imbriquées.
  ...
  - Peuvent modifier l'état avec setState.
...
**Note:** On utilisera Effect pour parler de l'événement fonctionalité offert par `useEffect`.
