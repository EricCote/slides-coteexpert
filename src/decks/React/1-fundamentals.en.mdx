...
- Or a _MetaFramework_ (Epic Stack, T3, Refine, Redwood, Meteor, Ionic, Wasp)
...
const element = <h3 title='alarm'>This alarm is set to {alarmTime}</h3>;
...
## JSX, behind the curtain (2/3)

JSX then gets compiled by Babel, swc, TypeScript or another tool:
...
Note: since React 17, transpilers use `_jsx` instead of `createElement`
