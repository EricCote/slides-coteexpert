...
## Charger avec un hook personalisé
...
## Conserver l'affichage des données grâce à useDeferred
...
- Un second avec la nouvelle valeur
  - Si ce second rendu complète immédiatement, on l'affiche (commit).
...
StartTransition indique un changement de State non-bloquant, qui n'est pas urgent. Si un autre événement lance un setState, le rendu de transition est interrompu (et redémmaré plus tard, un fois le traitement de l'événement terminé).
