...
- `<button onClick={() => alert('...')}>` passe la fonction flèche `() => alert('...')`
...
- Certains événements du naviagateur sont associés à un comportement.
...
- Keypress, scrolling lancent des comportements
...
- React fait un énorme travail pour cacher les incohérence des événements du DOM
- React n'utilise pas `addEventListener`
- React ne peut annuler un événement en retournant `false` (on doit utiliser `preventDefault()` ou `stopPropagation()` ) <hr />
- React utilise `onClick`, alors que le navigateur utilise `onclick`
- React gère le `onChange` pour chaque lettre tapée, mais le navigateur utilise `oninput`
- React nomme cela `onDoubleClick`, mais le navigateur utilise `ondblclick`
- REact n'a pas de `onLoad`
  <hr />
- Pour utiliser une version de React sans événements synthétiques, [Preact](https://preactjs.com)
