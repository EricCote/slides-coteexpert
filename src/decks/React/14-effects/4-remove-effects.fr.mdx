---
title: 'Éliminer les Effects inutiles'
---

# Éliminer les Effects inutiles

---

## Les Effects ne sont pas tous utiles

- Un Effect est une façon d'exécuter du code hors du paradigme de React
- Ça permet de synchroniser nos composants avec un système externe
  - Un widget non-React, un appel réseau, un apel au DOM
- Si on ne collabore pas avec un système externe, alors Effect est inutile
- Éliminer un effect inutile est une excellente idée
  - Plus simple à lire le code
  - Plus rapide
...
- Comment cacher des calculs coûteux sans Effects
- Comment réinitialiser l'état sans Effects
- Comment partager la logique entre gestionnaires d'événements
- Déplacer un Effect vers un gestionnaire d'événement
...
- Deux cas communs où le Effect n'est pas utile:
- **Transformer les données du rendu**
  ...
  - Mais ce n'est pas très efficace
    ...
    - React cédule le changement de state et un nouveau rendu
    - React fait un second rendu avec les données filtrées
  - Ces transformations devraient être appelées directement du rendu, sans Effect
- **Gérer un événement utilisateur**
  - On pourrait gérer le POST vers '/api/buy' avec un Effect.
  - Mais au moment de l'exécution de l'effect, il n'est plus possible de déterminer l'action exacte de l'utilisateur (ex: le bouton cliqué)
...
- Il faut des Effects, pour synchronisaer avec des librairies externes
  - Synchroniser un widget jQuery avec un état React
  - Otenir des données via fetch pour synchoniser une liste de résultats avec le champ de recherche.

---

## Problème: Utiliser un Effect pour modifier un prop ou un état

- Imaginons un `fullName` dérivant de `firstName` et `lastName`
- On pourrait imaginer gérer `fullName` dans un état, synchronisé via un Effect: (Mauvais)
...
- Mauvais: ceci génère des rendus additionnels
- un premier rendu avec le `fullName` moisi, et un second avec le `fullName` mis à jour
...
- Ce que l'on peut calculer (à partir de props ou d'état) ne devrait pas être stocké dans un état, mais plutôt calculé au rendu.
  - Plus rapide (on évite des rendus additionnels)
  - Plus simple
  - Moins de bogues

---

## Problème: Utiliser Effect pour faire du travail ou des calculs
...
On peut éliminer l'État et l'Effect
...
## Solution 2: Cacher le résultat quand le travail est long

Si `getFilteredTodos()` est lent ou qu'il y a beaucoup de `todos`, il faut limiter le nombre d'appels à la fonction

On peut cacher le résultat dans une mémoïsation avec `useMemo`
...
On n'exécute pas la fonction tant que `todos` or `filter` conservent leurs valeurs. On retourne plutôt le dernier résultat caché.

La fonction passée à `useMemo` doit être pure, car elle roule pendant un rendu.

---

## Déterminer si un calcul est long

- Si l'on créé ou l'on boucle sur des milliers d'objets, l'exécution peut être lente.
- On peut utiliser la console pour mesurer la durée d'une fonction
...
- Le résulat sera: `filter array: 0.15ms`
- Si la durée est significative (plus de `1ms`), on pourrait mémoïser avec `useMemo`
...
## Problème: Effect pour réinitialiser l'état quand prop change
...
## Solution: Passer une key différente créé un nouveau composant
...
## Problème: Effect modifiant l'état quand une prop est modifiée
...
## Solution 1: Modifier l'état dans le rendu (provoquant re-rendu)
...
## Solution 2: Éliminer le state redondant
...
## Problème: Effect exécute du code suite à un événement
...
## Solution: Appeler ce code dans le gestionnaire d'événements
...
### Problème: Utiliser Effect pour envoyer POST
...
## Solution: déplacer ce code vers un gestionnaire d'événements
...
### Problème: Effect exécute une chaîne de calculs
...
## Solution: Déplacer ce code dans le rendu et les événements
...
## Problème: Effect initialise l'application
...
## Solution: utiliser drapeau pour n'exécuter qu'une fois
...
## Solution 2: exécuter le code à l'initialisation du module
...
## Problème: Efect notifie le parent d'un changement d'état
...
## Solution: Déplacer la notification dans l'événement
...
## Solution 2: Remonter l'état local au composant parent
...
## Problème: Effect obtient des données qui sont passées au parent
...
## Problème: Effect s'abonne à un service externe côté client
...
## Problème: Effect fait un fectch sans logique de Cleanup
...
## Solution: ne pas oublier le cleanup avec Fectch
...
## Solution 2: Utiliser un Framework pour obtenir données
...
## Solution 3: Utiliser un Hook personnalisé
...
#### Transformer données sans Effect
...
#### Réinitialiser à zéro sans Effect
