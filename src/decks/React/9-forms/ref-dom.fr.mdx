...
Le button déplace le focus vers le `<input>`:
...
1. Declarer `inputRef` avec le hook `useRef`
...
## Example: Scroller vers un élément
...
Dans les exemples précédents, il y a un nombre spécifique de refs. Mais si on doit obtenir une ref pour chaque item d'une liste, que faire? Le code suivant ne fonctionne pas:
...
Cela échoue car les hooks doivent être appelés au **top-niveau** du composant.
On ne peut appeler `useRef` dans une boucle, une condition ou dans un appel `map()`.
...
Une autre solution, c'est de passer une fonction à l'attribut ref. C'est aussi appelé un "`ref` callback". React appelle ce callback du ref avec le node du DOM quand il faut générer la ref, et avec `null` quand c'est le temps de la vider. On peut alors alors utiliser un Tableau ou une Map, et accéder la ref par son index ou son ID.

Voici comment scroller dans une longue liste:
...
Quand on place une ref sur un composant génèrant un simple élément, tel que `<input>`, React place alors le node correspondant dans `ref.current`

Mais si on place une ref sur un composant JSX, tel que `<MyInput />`, on obtient alors `null`. (Le bouton ne fonctionne pas ci-bas).
...
C'est voulu. Manipuler le node d'un composant le rend encore plus fragile.
...
2. Le composant `MyInput` est déclaré avec un `forwardRef`. Il pourra recevoir `inputRef` et le placer dans l'argument `ref` (déclaré après les props).
...
Dans plusieurs sytèmes, c'est commun pour les composants de bas niveau (boutons, input, etc) de passer les refs aux nodes. Mais les composants plus haut niveau (formulaires, listes, sections de page) n'exposent pas leur Node pour éviter les dépendances accidentelles sur la structure du DOM.

---

## Exposer un sous-ensemble des API avec une référence impérative (imperative handle)

Dans l'exemple précédent, `MyInput` expose l'élément `input` du DOM. Ça permet au parent d'appeler `focus()`. Mais ça expose aussi au parent tout le reste du DOM (ex: changer les styles CSS). Pour limiter la fonctionnalité exposée, on utilise `useImperativeHandle`:
...
Explications useImperativeHandle
...
## Dépiler les mises à jour d’état de façon synchrone avec flushSync
...
- Le dernier "todo" est dans le DOM au moment où l'on tente de faire un défilement.

---

## Exemple de flushSync
...
Imaginons un message et deux boutons. Un bouton gère l'affichage avec React, en utilisant rendu conditionnel et l'état. L'autre gère l'affichage avec le DOM, et la méthode `remove()`. Basculez entre les 2 boutons.
...
Tenter d'utiliser setState pour montrer un élément qui a été enlevé manuellement du DOM génère un crash. React ne sait pas comment gérer un DOM qui a été modifié à l'extérieur de React.
...
Pas besin de refs sur toutes les images. Une seule ref vers l'image active, ou sur la liste elle-même, peut suffire. Utilisez `flushSync` pour s'assurer que le DOM est à jour avant le défilement.
...
## Focus le champ de saisie avec différents composants
