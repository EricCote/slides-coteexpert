...
`data` est une variable d'état (pour lire), et `setData` est la fonction setter (pour modifier).
...
- `useState` n'est pas le seul _hook_. Toute fonction démarrant par "`use`" est un _hook_.
...
- L'état (State) est une fonctionnalité, mais on en verra d'autres.
...
Même si les hooks sont des fonctions, vaut mieux considérer ceux-ci comme des déclarations des besoins de votre composant. On utilise les fonctionnalités React au "top niveau" d'un composant, de la même façon qu'on importe les modules au début des fichiers.
...
Si on veut que les deux galleries partagent leurs états? La bonne pratique en React, c'est d'enlever l'état aux composants enfants pour le rajouter à leur parent partagé. (On couvre cela plus tard.)
...
- Après le déclenchement du rendu, React appelle vos composants pour déterminer l'affichage.
...
C'est un processus récursif. Si le composant retourne d'autres composants, ils seront aussi appelés, ainsi que leurs enfants, et ainsi de suite. Le processus continue jusqu'à ce que tous les sous-composants soient rendus, et que React sache exactement quoi afficher.
...
- Ainsi que **tous ses sous-composants** (et ainsi de suite). Ça peut être lourd.
...
Par exemple, dans l'exemple suivant, le composant génère un re-rendu avec de nouvelles props à chaque seconde. On peut ajouter du texte au `<input>`, modifier la `value` du `<input>`, et le texte ne disparait pas au moment du re-rendu:
...
L'état est conservé par React, à l'extérieur de la fonction. Quand React appelle un composant, il offre un snapshot de l'état pour ce rendu particulier. Le composant retourne alors un snapshot du UI avec de nouveaux props et gestionnaires d'événements, générés à partir des valeurs d'états de ce rendu.
...
1. `setNumber(number + 1)`: `number` est `0` donc `setNumber(0 + 1)`.
   - React ne modifiera `number` à `1` qu'au prochain rendu.
...
**React ne regroupe pas à travers de _multiples_ événements intentionnels (comme de multiples clics).** Chaque clic est géré séparément. React ne regroupe que là où ça fait du sens. Cela nous assure que si le premier clic désactive le fomulaire, le second clic ne soumettra pas le formulaire une seconde fois.
...
Quand on appelle `useState` au prochain rendu, React passe à travers la file. L'état précédent `number` est `0`, React passe cette valeur à la première fonction updater (par le paramètre `n`).
...
- C'est après la série d'événements, au début du re-rendu, que React appelle la file des **updaters**.
...
Vous travaillez sur une application de marché d'euvres d'art où les usagers peuvent soumettre plusieurs commandes simultanément à une oeuvre. Chaque fois qu'un usager clique le boutton "Acheter", le compteur "achats en attente" doit augmenter de un. Après 3 secondes, le compteur "en attente" doit diminuer et le compteur "achats complétés" doit augmenter.
...
<div style={{ marginTop: '1em' }}>
  C'est trompeur. Quand le code exécute, la notion d'imbrication n'existe pas.
...
Immer est une façon de garder le code simple et concis, surtout avec des objets imbriqués, où la copie d'objet génère du code complexe.
...
|              | Mutation du tableau 🚫               | Nouveau Tableau ✅                           |
| ------------ | ------------------------------------ | -------------------------------------------- |
| Ajout        | `push()`, `unshift()`                | `concat()`, `[...arr]` syntaxe décomposition |
| Suppression  | `pop()`, `shift()`, `splice()`       | `filter()`, `slice()`, `toSpliced()` 🆕      |
| Remplacement | `splice()`, `arr[i] = x` assignation | `map()`, `.with()` 🆕                        |
| Tri          | `sort()`, `reverse()`,               | `toSorted()` 🆕, `toReversed()` 🆕           |
...
`filter()` génère un nouveau tableau d'`artists`.

---

## Transformer un tableau avec `map`

`map()` génère un nouveau tableau. La fonction passée à `map` détermine quoi faire avec chaque item, basé sur son contenu ou son index (ou les deux).
...
Les asignations du genre `arr[0] = x` mutent l'objet. On préfère utiliser `map` pour générer un nouveau tableau.

`map`, reçoit l'item et l'index comme arguments. On peut utiliser l'index (la position) pour décider si on retourne l'item original, ou autre chose.
...
Pour cela, on peut utiliser la méthode `slice()` avec la syntaxe de décomposition de tableau `...`. (Dans cet exemple, "Insérer" tente d'insérer à la position 1.)
...
On pourrait vouloir renverser un tableau, ou le trier. Mais les méthodes `reverse()` et `sort()` mutent le tableau orginal. Que faire si nous sommes avant 2023?

**Faites une copie du tableau d'abord, et faites des changements mutatifs ensuite**

On utilise la syntaxe de décomposition `[...list]` pour créer une copie du tableau original.
...
Par contre, même si on copie un tableau, on ne peut muter les objets du tableau. La copie est superficielle (shallow), et ce nouveau tableau contient les mêmes objets que l'original. Si on modifie un objet dans le tableau copié, on mute l'état.
...
Même si `nextList` et `list` sont deux tableaux distincts, **`nextList[0]` et `list[0]` pointent vers le même objet** En modifiant `nextList[0].seen`, on modifie aussi `list[0].seen`. C'est donc une mutation d'état. 🚫

On résout ce problème de la même façon qu'avec les objets JS imbriqués: en clonant plutôt que de muter.
...
Les objets ne sont pas réellement dans les tableaux. Même si on dirait qu'ils sont imbriqués, chaque objet est une valeur séparée, vers lequel le tableau pointe. **Lorsqu'on met à jour des états imbriqués, il faut créer des copies des objets à modifier, jusqu'au top niveau.**

Dans cet exemple, deux listes d'oeuvres d'art partagent le même état initial. À cause d'une valeur mutée, leur état est accidentellement partagé. Cocher la boite d'une liste affecte l'autre liste.
...
Même si le tableau `myNextList` est nouveau, les _items_ sont les mêmes que dans le tableau original `myList`. Changer `artwork.seen` modifie donc l'objet _original_. Cet objet se trouve aussi dans `yourArtworks`, causant le bogue. Ce bogue est difficile à trouver, mais il disparait dès qu'on arrête de muter.
...
Ici, `...` est la syntaxe de décomposition pour créer la copie d'un objet. Aucun des items d'état n'est muté, et le bogue est réglé.
...
En général, **le seul objet d'état que l'on peut muter, c'est celui qui vient tout juste d'être créé.** Si on insère une nouvelle oeuvre d'art, c'est correct de la muter. Mais si on modifie un objet provenant de l'état, il faut faire une copie.
...
- On pourrait réécrire la structure pour avoir une structure moins profonde (aplatir la structure)
