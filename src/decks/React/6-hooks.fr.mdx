...
useRef: Gère la persistance d'une valeur mutative, ne provoque JAMAIS de re-rendu
...
🆕 useTransition: Permet d'encapsuler des setState() (qui génèrent un re-rendu synchrone) de façon asynchrone, dans une transition. Cela permet de garder le reste de notre IU interactive. Un rendu de transition peut se faire interrompre sans problème. Permet aussi de gérer l'attente via une prop d'état de la transition, plutôt qu'un suspense.
