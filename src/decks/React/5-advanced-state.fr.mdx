...
- On pourrait lui donner un nom plus court avec une constante (un alias):
...
- De cette façon, la couleur est toujours synchronisée à la valeur passée par le parent.
...
- Ex: Quand il faut ignorer les mises à jour du prop
...
- On créé un clone de l'objet racine qui intègre le clone du parent ci-haut
...
Changez la structure de l'état pour permettre la sélection multiple. (Quelle serait la structure? réfléchssez avant de coder.) Chaque boite est indépendante des autres. Cliquer un élément sélectionné enlève la sélection. Le pied de page doit afficher le nombre d'éléments.
...
Pour n'avoir qu'un seul panel visible à la fois, que faire? (Sélectionner un second panel enlève la sélection précédente.)
...
La composante `Accordion` peut gérer l'état pour n'avoir qu'un seul `Panel` visible.
...
Dans ce cas, il n'y a qu'un seul paneau de visible. Plutôt qu'une valeur booléene, un nombre pourrait stocker l'index (la position) du paneau visible.
...
<Alert>
 Un composant est **"non-controllé"** quand il se gère lui-même, via un état interne. (Par exemple, le composant `Panel` original, avec son état booléen `isActive`, est non-controllé. Le parent ne peut influencer si le `Panel` est affiché ou non.)

Un composant est **"controllé"** quand l'information importante est gérée par un parent, via des props, plutôt que via son état interne. C'est le parent qui gère le comportement global. (ex: le composant `Panel` final, avec la prop `isActive` controllée par le composant `Accordion`)
...
- Ça ne signifie pas que tout l'état est stocké à un seul endroit.
...
Quand un composant possède un état, on pourrait croire que cet état est stocké dans le composant.
...
Les clés (keys) sont utiles pour le rendu de listes. Mais les clés ne sont pas seulement pour les listes. On peut utiliser des clés pour que React distingue entre les composants.

<Alert>
  Par défaut, React utilise le positionnement pour distinguer entre les composants. Mais les clés permettent de distinguer par le `key` plutôt que le positionnment. 
  
  Ça permet de gérer le changement d'ordre, l'insertion ou la suppression d'items dans une liste. React se fie sur la clé plutôt que la position quand `key` est spécifié.

</Alert>

Dans notre app, plutôt que de distinguer entre le
Counter de la première position et de la seconde position, on peut maintenant spécifier le counter dans la clé. Par exemple, la clé de _Justin_.
...
Les clés n'ont pas besoin d'être globalement uniques. Elles doivent seulement être unique **_sous le parent_.**
...
- React conserve l'état tant que le composant est rendu à la même position
- L'état n'est pas conservé dans le JSX, c'est associé à la position du JSX dans l'arborescence
- On peut forcer une partie de l'arborescence à réinitialiser l'état en chageant le `key`
- Ne jamais imbriquer les composant, car cela réinitialise l'état
...
On peut inverser l'affichage. Mais il y a un bogue. Le contenu de `<Field>` ne change pas de place, même si le `label` est inversé
...
Dans cette de contact, on peut faire "save" pour sauegarder le contact, et "reset" pour ramener le contact à la dernière sauvegarde.

Bogue: Quand on sélectionne un contact différent, l'état change mais le formulaire ne change pas. On doit réparer.
...
On clique "Next", le navigateur démarre le chargement. Mais pendant le chargement l'ancienne image est toujours affichée, parce qu'on réutilise `<img>`. Ce qu'on préfèrerait, c'est que dès que le nouveau texte apparait, l'image existante disparaisse.

<Hint>On veut que React remplace le DOM plutôt que de le réutiliser</Hint>
...
Chaque `Contact` a un état qui indique si on a fait "Show email". Mais si on renverse l'ordre, les noms changent de place, mais pas la position du "Show email". Il faut que les courriels suivent les contacts, quel que soit l'ordre.
