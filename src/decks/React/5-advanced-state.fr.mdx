...
Dans les deux cas, **il faut modifier des variables d'état (state) pour mettre à jour l'IU.**
...
On représente l'état visuel du composant avec `useState`. On garde cela simple.
...
Il faut aussi ajouter les variables qui représentent l'état visuel. Ici, 4 valeurs booléennes:
...
Il faut réessayer quelques fois avant d'arriver à un "state" optimal.

---

### Étape 4 : Éliminer toute variable d'état non-nécessaire

On refactorise l'état pour ne garder que l'essentiel.
...
- **Un état provoque-il un paradoxe?** `isTyping` et `isSubmitting` ne peuvent simultanément être `true`. Un paradoxe indique que l'état manque de contraintes. Il y a 4 cominaisons de 2 booléens, mais seulement 3 états valides. On élimine l'état "impossible", avec un `status` de 3 modes: `'tape'`, `'envoi'`, ou `'succès'`.
- **La même info est-elle disponible ailleurs dans l'état?** un autre paradoxe: `isEmpty` et `isTyping` ne peuvent être `true` simultanément. On élimine `isEmpty` et on valide si `answer.length === 0`.
...
- Exemple, un état `error` non-null n'est pas simultanément possible avec un `status` de `succès`.
...
Un autre exemple d'unification: un tableau ou objet pour stocker les données d'un formulaire où l'utilisateur ajoute des champs de saisie personnalisés.
...
On peut ajouter des constantes pour aider la lecture. Ce ne sont pas des variables d'état, nul besoin de craindre un état impossible.
...
Si on peut calculer une info à partir de **props** ou de **l'état**, cette info ne devrait **PAS** se trouver dans l'état.
...
C'est parfois utile de dupliquer une prop dans l'état. Ex: quand il faut ignorer les mises à jour de ce prop.
...
Si on clique "Choose" sur un item, et qu'on le modifie par la suite, l'item de la liste est mis à jour, mais l'item sélectionné en bas de la liste demeure le même. Nous avons avons un état dupliqué, quand on met à jour la liste, `selectedItem` demeure inchangé.
...
(Alternative: utiliser `selectedIndex` plutôt que `selectedId`, pour retrouver l'item par sa position dans la liste.)
