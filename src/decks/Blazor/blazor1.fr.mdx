...
Pour exécuter en mode "hot-reaload":
...
Dans la racine du projet, on ajoute un fichier `TodoItem.cs`.

`Title` un string nullable (avec le `?`)
...
Et on ajoute aussi l'attribut @bind
