...
- **Visual Studio Code**
  ...
  - Extensions VS Code: C# Dev Kit...

---

## Comment travailler avec VS Code

- Installer `C# Dev Kit`
- Configurer les paramètres de VS Code avec <kbd>ctrl-,</kbd>
  - Text Editor: Tab Size: 2
  - Text Editor: Formatting: Format On Save: true
  - Extensions: C#: OmniSharp: Format: Enable: true

---

## Comment travailler avec VS Code

- <kbd>Ctrl-Shift-B</kbd> pour faire un build
- Le build est configuré dans le fichier `.vscode/tasks.json`

<hr />

- <kbd>F5</kbd> pour démarrer le déboggage.
- Le débogage est configuré dans le fichier `.vscode/launch.json`

<hr />

- Allez en mode commande avec <kbd>Ctrl-Shift-P</kbd>
- Utilisez la commande : "DOTNET: Generate Assets for Build and Debug" pour générer ces fichiers
...
Dans un dossier vide, on lance la commande suivante:

```bash
dotnet new blazor -f net8.0
```

Si on veut créer un nouveau dossier:

```bash
dotnet new blazor -o MonDossier --no-https true -f net8.0
```

`--output MonDossier` est le dossier de sortie (output)
`--no-https true` désactive le port https
`--framework net8.0` pour choisir la version .NET
...
Créer un dossier `projets` sur le disque D:
...
Se déplacer dans le dossier `projets` et démarrer VS Code:
...
Le point (.) signifie d'ouvrir le dossier courant.
...
En VS2022, on utilise le menu contextuel pour ajouter la page Todo dans le dossier
`Components/Pages`
...
Dans la racine du projet, on ajoute un fichier classe `TodoItem.cs`. Ajoutez les code suivant:
    ...
    @foreach (var todo in todos)
    {
        <li class="list-unstyled">
            <input type="checkbox" @bind="todo.IsDone"
                   class="form-check-input align-text-bottom d-inline-block" />
...
On modifie la balise `h3` existante
