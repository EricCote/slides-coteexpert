...
- Une API Web côté serveur est une interface de programmation
  ...
  - exposée sur le web via un server web https
...
-Il y a 2 paradigmes: **Controllers** et **Minimal APIs**

import '../React/table-rows.css';

<div className="my-tbl">

| Comparer                      | Controllers                      | Minimal API                                |
| ----------------------------- | -------------------------------- | ------------------------------------------ |
| Orientation                   | Orienté-Objet                    | Orienté-Extensions                         |
| Où                            | Classe dérivée de ControllerBase | Methodes d'extensions avec classes cachées |
| Action                        | via Méthode                      | via lambda                                 |
| Injection de dépendances (DI) | via constructeur                 | via Fournisseur de service                 |
| Configuration                 | par attributs                    | par Fournisseur de service                 |
| Maturité                      | existe depuis 2012               | existe depuis 2021                         |
...
- **Routage spécifié dans le code.** On déclare nos routes et verbes dans des attributs qui décorent notre code. Les paramètres de nos méthodes peuvent spécifier leur provenance via un attribut: un élément du chemin, le _query string_ ou le _request body_.
...
- On peut utiliser l'extension **.http**
  - Utilise l'extension "Rest Client" en VS Code
...
- On dérive de `ControllerBase`
  - En MVC, on hérite de `Controller`, car cela gère les vues
  - En Web API, il n'y a pas de vue, il faut donc utiliser `ControllerBase`
