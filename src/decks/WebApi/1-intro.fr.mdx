...
- **.NET Runtime 9.0.3**: Environnement d'exécution (runtime, libs) pour les apps CLI (34MB)
...
- Il y a plusieurs sous-technologies:
  ...
  - **Blazor**: Apps web interactives en C#
    ...
    - **Blazor WebApp** (2024+): Interactions flexibles. Ex: Interactions sur le serveur d'abord, bascule sur le client après chargement du WASM
...
Démarrez machine virtuelle

Personnalisez la machine. (On peut changer la langue, on peut changer le mode "dark")

Lancez _Visual Studio Code_ (icône bleu) et démarrez une fenêtre de terminal (menu "..." -> Terminal -> New Terminal)

Lancez commande `dotnet --list-sdks` (le SDK 9.x est installé)

Lancez commande `dotnet --list-runtimes` (les runtimes ASP.NET Core, .NET Core et Desktop sont installés pour 8.x et 9.y)

Dans la barre d'activité (barre verticale d'icônes), choisir _Extensions_ (blocs empilés) et rechercher "C# Dev Kit". Installez le "C# Dev Kit"

Créez un certificat local pour le developpement https. Lancez commande `dotnet dev-certs https -t`

Fermez VSCode

Démarrez Visual Studio 2022 (Icône magenta).

Quand on demande de vous identifier, vous pouvez faire "skip"

Pour l'écran de personnalisation, vous pouvez garder les défauts (General et thème Dark)

Pour l'écran de démmarage, sélectionnez "create a new Project"

Sélectionnez un projet Web API en .NET 8, choisir les défauts

Testez le projet en mode déboggage, visitez `/weatherforecast`, visitez `/swagger`
